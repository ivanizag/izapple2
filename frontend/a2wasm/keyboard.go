...
func (k *wasmKeyboard) putKey(key ebiten.Key) {
	...
	*/
	...
	case ebiten.KeyPrintScreen:
		if ctrl {
			// Screenshot test mode disabled in WASM
			fmt.Println("Screenshot test mode not available in web version")
		} else {
			// Save screenshot through JavaScript bridge
			saveScreenshot(k.a)
		}
...
// GetKey returns a pressed key
func (k *wasmKeyboard) GetKey(strobe bool) (key uint8, ok bool) {
	return k.keyChannel.GetKey(strobe)
}
